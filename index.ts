...
while((f = args[i])) {
  ...
  else if(f.substring(0,7) === 'config=') opts['config'] = f.substring(7)
  else if(f === '-c') {
    opts['config'] = args[++i]
  }
  else if(f.substring(0,3) === '-c=') opts['config'] = f.substring(3)

  else if(f.substring(0,8) === '--title=') {
    title = f.substring(8)
  }
  else if(f.substring(0,7) === '--title') {
    title = args[++i]
  }
...
export async function execute() {
  ...
  // now remove the Toc from the markdown file
  capToc()
}
...
function showHelp() {
  ...
  console.log(ac.bold.black(" -c=<file>"), ac.black.italic("-- same as above"))
  console.log('')
  console.log(ac.bold.black("--title <title>"), ac.black.italic("title to appear in API doc (markdown only)"))
  console.log(ac.bold.black("--title=<title>"), ac.black.italic("-- same as above"))
  console.log('')
...
// ##DH-OFF
const attrib = `<span style="float:right; margin-top:-15px; font-size:smaller; color:darkred;">Documentation generated by <a href=https://github.com/tremho/docholiday>Doc-Holiday</a></span>`
// ##DH-ON
 ...
 */
export function capToc() {
  let apiFile = config.markdown
  if(fs.existsSync(apiFile)) {
    try {
      let contents = fs.readFileSync(apiFile).toString()
      let v = contents.indexOf('<!-- Generated by documentation.js.')
      if(v !== -1) {
        v = contents.indexOf('### Table of Contents')
        if(v !== -1) {
          v = contents.indexOf('#', v+4)
        }
      }
      contents = docHead + contents.substring(v)
      fs.writeFileSync(apiFile, contents)
    } catch(e) {
